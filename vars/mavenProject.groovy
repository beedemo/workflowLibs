...
def call(body) {
    ...
    stage('create/update build image') {
        node {
            ...
            try {
                sh "docker inspect -f . beedemo/${config.repo}-build"
                ...
                if(rebuildBuildImage){
                    ...
                    sh "docker run -u 1000:1000 --name maven-build -w ${workspaceDir} --volumes-from ${nodeContainerId} beedemo/${config.repo}-build mvn -Dmaven.repo.local=/maven-repo ${mvnBuildCmd}"
                ...
                def shCmd = "docker run -u 1000:1000 --name maven-build -w ${workspaceDir} --volumes-from ${nodeContainerId} kmadel/maven:${mavenVersion}-jdk-${jdkVersion} mvn -Dmaven.repo.local=/maven-repo ${mvnBuildCmd}"
                echo shCmd
                sh shCmd
                //create a repo specific build image based on previous run
                sh "docker commit maven-build beedemo/${config.repo}-build"
